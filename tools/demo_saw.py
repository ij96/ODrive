...
# t0 = time.monotonic()
# while True:
#     setpoint = 10000.0 * math.sin((time.monotonic() - t0)*6)
#     print("goto " + str(int(setpoint)))
#     my_drive.motor0.set_pos_setpoint(setpoint, 0.0, 0.0)
#     time.sleep(0.01)
my_drive.motor0.config.vel_limit = 10000
amp = 6000
my_drive.motor0.pos_setpoint = amp
print("Position setpoint is " + str(my_drive.motor0.pos_setpoint))
time.sleep(3)
while True:
	while (int(my_drive.motor0.encoder.pll_pos) >= 6000*0.95):
		my_drive.motor0.pos_setpoint = -amp
	while (int(my_drive.motor0.encoder.pll_pos) <= -6000*0.95):
		my_drive.motor0.pos_setpoint = amp
